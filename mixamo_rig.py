...
class MR_OT_make_rig(bpy.types.Operator):
    ...
    def draw(self, context):
        layout = self.layout
        layout.prop(self, 'bake_anim', text="包含动画")        
        layout.prop(self, 'ik_arms', text="IK 手臂")
        layout.prop(self, 'ik_legs', text="IK 腿部")
...
class MR_PT_MenuMain(Panel, MixamoRigPanel):
    ...
    def draw(self, context): 
        ...
        #col.scale_y = 1.3
        #col.prop_search(scn, "mix_source_armature", scn, "objects", text="骨骼")
        arm_name = "None"
...
class MR_PT_MenuRig(Panel, MixamoRigPanel):   
    ...
    def draw(self, context):
        ...
        col.operator(MR_OT_make_rig.bl_idname, text="创建控制器")
        col.operator(MR_OT_zero_out.bl_idname, text="清空所有姿态")
        
        col = layt.column(align=True)
        col.separator()

        if bpy.context.mode != 'EDIT_MESH':
            col.operator(MR_OT_edit_custom_shape.bl_idname, text="编辑控制器形状")
        else:
            col.operator(MR_OT_apply_shape.bl_idname, text="应用控制形状")
...
class MR_PT_MenuAnim(Panel, MixamoRigPanel):  
    ...
    def draw(self, context):
        ...
        col = layt.column(align=True)         
        col.scale_y = 1        
        #col.prop_search(scn, "mix_target_armature", scn, "objects", text="控制器")
        col.label(text="骨架源:")
        ...
        col = layt.column(align=True)         
        col.scale_y = 1.3
        col.operator(MR_OT_import_anim.bl_idname, text="将动画应用到控制器")        
      
        col = layt.column(align=True)     
        col.scale_y = 1.3
        col.operator(MR_OT_bake_anim.bl_idname, text="烘焙动画")
...
class MR_PT_MenuUpdate(Panel, MixamoRigPanel):  
    ...
    def draw(self, context):
        layt = self.layout        
        layt.operator(MR_OT_update.bl_idname, text="更新控制器")
...
class MR_PT_MenuExport(Panel, MixamoRigPanel):  
    ...
    def draw(self, context):
        layt = self.layout        
        layt.operator('export_scene.gltf', text="导出 GLTF")#MR_OT_exportGLTF.bl_idname
